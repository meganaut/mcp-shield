...
use mcpshield_standard::vault::SqliteVaultBackend;
...
async fn make_db() -> Arc<dyn mcpshield_db::Store> {
...
impl mcpshield_core::vault::VaultBackend for NoopVault {
    ...
    fn get_token(
        &self, _: &str,
    ) -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<Option<VaultTokenData>, mcpshield_core::error::CoreError>> + Send + '_>> {
...
async fn test_multi_integration_routing() {
    ...
    let policy = Arc::new(CachingPolicyEngine::new(Arc::new(
        mcpshield_standard::noop::NoopPolicy,
    )));
...
async fn test_policy_namespaced_isolation() {
    ...
    db.upsert_policy_rule(&mcpshield_db::PolicyRule {
...
async fn test_vault_round_trip() {
    ...
    db.insert_integration(&mcpshield_db::Integration {
