...
use mcpcondor_core::types::{AgentId, AuditEvent, AuditOutcome, IntegrationId, McpPrimitive};
...
pub async fn authenticate_bearer(
    ...
    let token = match extract_bearer_token(headers) {
        ...
        None => {
            return Err(axum::http::Response::builder()
                ...
                .header("www-authenticate", "Bearer realm=\"MCPCondor\"")
    ...
    match state.db.get_token_by_hash(&token_hash, now).await {
        ...
        Ok(None) => Err(axum::http::Response::builder()
            .status(StatusCode::UNAUTHORIZED)
            .header("content-type", "application/json")
            .header("www-authenticate", "Bearer realm=\"MCPCondor\", error=\"invalid_token\"")
            .body(axum::body::Body::from(
...
async fn handle_tools_call(state: Arc<AppState>, req: JsonRpcRequest, agent_id: String, client_id: Option<String>) -> Response {
    ...
    let params: mcpcondor_core::mcp::ToolCallParams =
